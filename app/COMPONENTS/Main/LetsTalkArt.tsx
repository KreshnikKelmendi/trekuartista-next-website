...
export default function LetsTalkArt() {
  ...
  // Wait until mounted on the client to run calculations safely
  useEffect(() => {
    setIsMounted(true);
  }, []);

  // 1. ULTRA-SMOOTH SCROLL LOGIC
  const { scrollYProgress } = useScroll({
    target: isMounted ? containerRef : undefined,
    offset: ["start end", "end start"],
  });

  // We create a "Spring" to make the scroll movement feel liquid/smooth
  const smoothProgress = useSpring(scrollYProgress, {
    stiffness: 40,   // Lower stiffness = more "liquid" feel
    damping: 20,     // Higher damping = less bouncy
    restDelta: 0.001
  });

  // This handles the text "drifting" upward as you scroll.
  // The range [120, -120] determines the travel distance.
  const yLift = useTransform(smoothProgress, [0, 1], [120, -120]);

  // 2. TEXT ANIMATION VARIANTS (SLOW & SMOOTH)
  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.08, // Increased for a slower, intentional build
        delayChildren: 0.2,
      },
    },
  };
  ...
  const letterVariants = {
    hidden: { 
      filter: "blur(12px)", 
      opacity: 0, 
      y: 40 // Starting lower for a longer glide up
    },
    visible: {
      filter: "blur(0px)",
      opacity: 1,
      y: 0,
      transition: { 
        duration: 1.2, // Much slower animation (1.2 seconds)
        ease: [0.16, 1, 0.3, 1] as const
      },
    },
  };

  const text1 = "LET'S TALK ART";

  return (
    <section 
      ref={containerRef} 
      className="w-full pt-6 pb-12 lg:py-12 px-5 lg:px-[55px] overflow-hidden flex items-center"
    >
      <motion.div 
        style={{ y: isMounted ? yLift : 0 }} // Prevent SSR transformation jumps before mount
      ...
      >
        <div className="self-start leading-[0.85] select-none">
          {/* LINE 1 */}
          <div className="flex flex-wrap overflow-hidden py-4">
            {text1.split("").map((char, index) => (
              <motion.span
                key={`line1-${index}`}
                variants={letterVariants}
                className="inline-block font-custom text-7xl lg:text-[7rem] 2xl:text-[9rem] leading-none lg:leading-[0.9] uppercase tracking-[-0.02em] text-black"
              >
                {char === " " ? "\u00A0" : char}
              </motion.span>
            ))}
        ...
        {/* SPINNING ICON (Slower rotation for elegance) */}
        <motion.div
          variants={letterVariants}
          className="relative h-[130px] w-[130px] shrink-0 self-end lg:h-[180px] lg:w-[180px] lg:self-auto"
        >
          <motion.svg
            animate={{ rotate: 360 }}
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <svg width="24" height="28" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8 1V15" stroke="#3110EE" strokeWidth="2" strokeLinecap="round" />
              <path d="M2 10L8 16L14 10" stroke="#3110EE" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </motion.div>
      </motion.div>
